...
package org.vertx.java.framework.testng;
...
public class TestUtils {
  ...
  public void azzert(boolean result, String message) {
    ...
    public void azzertResponseSuccess(Message<JsonObject> event) {
        if (!event.body.getString("status").equals("ok"))
            azzert(false, event.body.getString("error"));
    }
    public <T> void azzertEquals(JsonObject jsonObject, String actualMapName, String actualFieldName, T expected) {
        azzert(jsonObject!=null,"null json in azzert");
        azzert(actualMapName!=null,"null actualMapName in azzert");
        azzert(actualFieldName!=null,"null actualFieldName in azzert");

        JsonObject object = jsonObject.getObject(actualMapName);
        azzert(object!=null,"json object does not contain a json object in property "+actualMapName);

        Map<String, Object> result = object.toMap();
        Object actual = result.get(actualFieldName);
        azzertEquals(expected, actual);
    }
    public <T> void azzertEquals(JsonObject jsonObject, String actualFieldName, T expected) {
        azzert(jsonObject!=null,"null json in azzert");
        azzert(actualFieldName!=null,"null actualFieldName in azzert");

        Object actual = jsonObject.toMap().get(actualFieldName);
        azzertEquals(expected, actual);
    }

    public <T> void azzertEquals(T actual, T expected) {
        azzert(
                (expected == null && actual == null) ||
                        (expected != null && expected.equals(actual)),
                String.format("Expected %s of type %s, actually was %s of type %s", expected,expected==null?"null":expected.getClass().getSimpleName(), actual,actual==null?"null":actual.getClass().getSimpleName()));
    }
  ...
  public void appStopped() {
    ...
    public void testFail() {
        sendEvent(EventFields.ASSERT_RESULT_VALUE_FAIL);
    }
